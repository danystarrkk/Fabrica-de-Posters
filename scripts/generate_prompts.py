...
def ask_ollama(model, prompt, expected_count):
    payload = {
        "model": model,
        "prompt": prompt,
        "stream": False,
...
def build_generation_prompt(batch, amount):
...
def main():
    ...
    for batch_index, (batch, amount) in enumerate(zip(batches, distribution), start=1):
        ...
        # Si por ejemplo hay más grupos que prompts,
        # algunos grupos pueden recibir 0.
        if amount == 0:
            continue
        ...
        print(f"Generating {amount} prompt(s)...")
